...
fetch_error = None
try:
    fetch_ohlcv(cfg.exchange, cfg.symbols[0], cfg.timeframe, 1, cfg)
...
with st.sidebar:
    st.header("Settings")
    st.markdown(f"**Exchange:** {cfg.exchange}")
    st.markdown(f"**Timeframe:** {cfg.timeframe}")
    st.markdown(f"**Timeout:** {cfg.network.timeout_ms} ms")
...
if fetch_error:
    st.warning(
        "Initial data fetch failed. Run `python scripts/selftest_connection.py` or configure proxies/timeouts."
    )
